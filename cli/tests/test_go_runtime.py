...
class TestGoRuntime:
    def test_detect_with_go_mod(self, go_runtime, tmp_path):
        ...
        with patch("xbin.runtimes.go.shutil.which", return_value="/usr/bin/go"):
            with patch("xbin.runtimes.go.subprocess.run") as mock_run:
    ...
    def test_detect_no_go_on_path(self, go_runtime, tmp_path):
        ...
        with patch("xbin.runtimes.go.shutil.which", return_value=None):
            with pytest.raises(ValueError, match="no go on PATH"):
