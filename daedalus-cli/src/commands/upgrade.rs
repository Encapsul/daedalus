...
const GITHUB_API: &str = "https://api.github.com/repos/Tednoob17/daedalus/releases/latest";
...
pub fn run(args: UpgradeArgs) -> Result<()> {
    ...
    if args.verbose {
        eprintln!("[daedalus] current version: {current}");
    }

    let latest = fetch_latest_version().context("failed to fetch latest version")?;

    if args.verbose {
        eprintln!("[daedalus] latest version:  {latest}");
    }
    ...
    let url = format!("https://github.com/Tednoob17/daedalus/releases/download/{tag}/{asset}");
    ...
    // Download
    if args.verbose {
        eprintln!("[daedalus] downloading {asset}...");
    }
    ...
    if args.verbose {
        eprintln!("[daedalus] checksum verified");
    }
    ...
    let daedalus_path = find_daedalus_binary()?;
    let install_dir = daedalus_path
        .parent()
        .context("cannot determine install directory")?;

    if args.verbose {
        eprintln!("[daedalus] installing to {}...", install_dir.display());
    }
    ...
    if needs_sudo && !args.force {
        ...
        eprint!(
            "[daedalus] install to {} requires sudo. Continue? [y/N] ",
            install_dir.display()
        );
...
fn find_daedalus_binary() -> Result<PathBuf> {
    ...
    // Try which
    if let Ok(p) = which::which("daedalus") {
        return Ok(p);
    }

    anyhow::bail!("cannot locate daedalus binary for self-update")
}
...
fn is_writable(path: &Path) -> bool {
    ...
    let probe = dir.join(format!(".daedalus-probe-{}", std::process::id()));
