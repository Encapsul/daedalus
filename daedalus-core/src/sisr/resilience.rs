...
//! depends on the `.daedalus.bak` snapshot being trustworthy at every instant:
...
/// `/dir/app.daedalus` → `/dir/app.daedalus.bak`.
///
...
pub fn create_backup(src: &Path, backup: &Path) -> io::Result<()> {
    ...
    let mut writer = AtomicWriter::new(parent, "daedalus.bak")?;
...
pub fn restore_backup(bin: &Path, backup: &Path) -> io::Result<()> {
    ...
    let mut writer = AtomicWriter::new(parent, "daedalus.restore")?;
    io::copy(&mut src_file, writer.file_mut())?;
...
mod tests {
    ...
    fn backup_path_is_co_located() {
        ...
        assert_eq!(
            backup_path_for(bin),
            PathBuf::from("/home/u/app.daedalus.bak")
        );
