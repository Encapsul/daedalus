...
//!
//! [`upgrade_binary`] converts a classic `.daedalus` (no `FLAG_SISR`) into a
...
pub fn upgrade_binary(
    ...
    if payload_end > meta_start || meta_end > tail {
        return Err(err(
            "malformed .daedalus: overlapping or out-of-bounds segments",
        ));
    }
...
mod tests {
    use super::*;
    use crate::assembly::{assemble_daedalus, AssemblyInput};
    ...
    fn upgrade_preserves_segments_and_sets_sisr() {
        ...
        let out = tmp.path().join("upgraded.daedalus");
        ...
        let remote_path = tmp.path().join("upgraded.daedalus.manifest");
    ...
    fn upgrade_rejects_already_sisr() {
        ...
        .unwrap();
        let err = upgrade_binary(&input, &tmp.path().join("out.daedalus"), &config()).unwrap_err();
    ...
    fn upgrade_rejects_signed_input() {
        ...
        data[last + 5] = 3;
        let signed = tmp.path().join("signed.daedalus");
        fs::write(&signed, &data).unwrap();
        let err = upgrade_binary(&signed, &tmp.path().join("out.daedalus"), &config()).unwrap_err();
    ...
    fn upgrade_rejects_non_daedalus() {
        ...
        fs::write(&input, b"this is not an daedalus file at all").unwrap();
        assert!(upgrade_binary(&input, &tmp.path().join("out.daedalus"), &config()).is_err());
    }
}
