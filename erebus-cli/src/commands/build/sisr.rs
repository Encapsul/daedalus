...
pub(crate) fn build_sisr_config(
    key_path: &Option<PathBuf>,
    embed_model: bool,
) -> Result<SisrBuildConfig> {
...
mod tests {
    ...
    fn build_sisr_config_defaults_to_64k_chunks_without_key() {
        let cfg = build_sisr_config(&None, false).unwrap();
        assert!(cfg.enabled);
    ...
    fn build_sisr_config_uses_16mb_chunks_with_embed_model() {
        let cfg = build_sisr_config(&None, true).unwrap();
        assert_eq!(cfg.chunk_target_size, 16 << 20);
    }
    ...
    fn build_sisr_config_loads_32_byte_key() {
        ...
        let cfg = build_sisr_config(&Some(key_path), false).unwrap();
    ...
    fn build_sisr_config_rejects_wrong_key_length() {
        ...
        assert!(build_sisr_config(&Some(key_path), false).is_err());
    }

    #[test]
    fn build_sisr_config_errors_on_missing_key() {
        let path = PathBuf::from("/nonexistent/key.bin");
        assert!(build_sisr_config(&Some(path), false).is_err());
    }
}
