...
#[derive(Args)]
pub struct RunArgs {
    /// Path to the .erebus file to execute
    pub file: PathBuf,
...
pub fn run(args: RunArgs) -> Result<()> {
    ...
    // Verify it's a valid erebus file before executing
    {
        use erebus_core::format::Footer;
        let mut f = std::fs::File::open(&file)
            .with_context(|| format!("failed to open {}", file.display()))?;
        Footer::read_from(&mut f).context("invalid .erebus file — bad footer")?;
    }
    ...
    let err = Command::new(&file)
        .args(std::env::args_os().skip(3)) // skip: erebus run <file>
        .status()
