...
>
> **Work session 2026-08-11 (audit follow-up):** resolved #13, #1, #29, #17. #13 expands the seccomp denylist with `bpf` and `userfaultfd` (page-fault oracle / kernel-inspection escalation paths not covered by namespaces); the filter instruction-count test was updated and a new membership assertion added. #1 corrects the stale threat model: encryption uses a fresh random key stored in metadata (not derived from the signing seed), and `--encrypt` is now documented + help-warns that it is obfuscation-only, not confidentiality. #29 folds a canonical config fingerprint into the build cache key (encrypt/sign/isolation/seccomp/landlock/env/squashfs/compression/otel/…) so mismatched flags can no longer serve a stale artifact, and moves the Ed25519 signing step before the cache store so hits return complete (signed) binaries; also serializes the XDG_CACHE_HOME env-var mutation in the cache unit tests with a global mutex to fix a parallel-test race. #17 makes the remaining hardcoded launcher constants configurable: PHP dev-server port (`XBIN_PHP_PORT`), decompression-bomb extraction limits (`XBIN_MAX_EXTRACT_SIZE`/`XBIN_MAX_EXTRACT_FILES`, read once per extraction from the launcher env, never from the untrusted payload), and the extraction-cache GC cap (`XBIN_CACHE_MAX_ENTRIES`); the health-gate timeout was already env-overridable. All env knobs are launcher-only, so attackers cannot weaken limits by smuggling values into the `.xbin`.
>
> **Work session 2026-08-11 (night):** resolved #40, #33, #36, #3, #12, #15, #41, #4. #40 rejects unknown runtimes in the stub instead of silently launching under bash (`Runtime::from_name` + early check + full interpreter map incl. electron/dotnet/hugo/wasm). #33 warns at build + runtime when `--seccomp`/`--landlock` are set below isolation 2. #36 dropped the `/tmp/xbin-build-tools` deletion (symlink/TOCTOU). #3 pins `ed25519-dalek >= 2.1.0` (CVE-2023-48022) + CI lockfile guard. #12 exposes `XBIN_HTTP_TIMEOUT_{CONNECT,RESPONSE,BODY}`. #15 serializes cache GC with `flock` and only evicts `.ready`-marked caches. #41 deleted the dormant layered cache/chunked-decrypt paths (`cache_key_v2`, `slice_layers`, `chunked_decrypt_aes_gcm`, `meta.layers`). #4 audit: all secret comparisons already constant-time (ed25519-dalek `verify`, XOR-fold `ct_eq_sha256`); SISR chunk-hash checks in `sisr/engine.rs` switched to `subtle::ConstantTimeEq` for policy consistency. #45 partially addressed: the at-rest signing gap is documented (`engine.rs::apply_update_with_stats` doc + CLI warning on every SISR build); the signature chain is deferred.

> **Work session 2026-08-11 (refactor):** resolved #9 and #7. #9 replaces the three 8-parameter `assemble_xbin` / `assemble_xbin_with_sisr` / `assemble_xbin_with_sisr_artifacts` entry points with a single `assemble_xbin(&Path, &AssemblyInput)` gated by an `AssemblyInput` config struct (folding the optional SISR stage into the struct), splitting the body into `sisr_footer_ext` / `build_footer` / `write_footer` helpers to stay under the 30-line ceiling; all call sites (build CLI, python bindings, sisr engine, e2e fixtures, cli_tests) were updated. #7 audits every `unsafe` site in the stub: all FFI/syscall blocks already carry a `// SAFER:` comment — no `static mut` remains and no `unsafe` lives outside FFI/syscall calls — so the safe/unsafe boundary is closed.
