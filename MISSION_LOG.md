...
> `daedalus-core`, pas de dossier `crates/` (tout va dans `daedalus-core/src/`),
...
### CLI — `daedalus upgrade-binary`

- `daedalus-cli/src/commands/upgrade_binary.rs` + sous-commande dans `main.rs`.
...
- `legacy_binary_runs_on_v2_runtime` : binaire v1 (assemblé sans SISR) exécuté
  par le vrai stub ⇒ sortie OK, zéro warning `[daedalus]` (§5, §11).
...
- `stub/tests/e2e_sisr/mod.rs` : helpers partagés — `TestEnv`, `env(v1, v2)`
  (vrai binaire `CARGO_BIN_EXE_daedalus-stub`, trusted keys, update stagé signé),
...
- `stub/tests/e2e_sisr/mock_server.rs` : `MockHttpServer` 100 % std (TcpListener
  ...
  `DAEDALUS_TRUSTED_DIR`, `DAEDALUS_HEALTH_TIMEOUT_MS`.
...
- `daedalus-core/src/sisr/network_test.rs` : `FaultInjectingFetcher` (wrapper du
...
- `proptest = "1"` en dev-deps de `daedalus-core`.
...
2. **Health Gate = superviseur fork+waitpid**, pas un exec direct : l'exec seul
   ...
   `waitpid(WNOHANG)` 50 ms jusqu'à `DAEDALUS_HEALTH_TIMEOUT_MS`. Exited 0 ou
...
4. **Quarantaine** quand `attempts >= DAEDALUS_HEALTH_MAX_ATTEMPTS` (défaut 3,
...
6. **Store = JSON par version** dans `~/.cache/daedalus/health/<sha256>.json`
   (`XDG_CACHE_HOME`-relatif), écritures atomiques (tmp+rename) ; version id =
   `footer.sha256_hex()` = SHA-256(payload ‖ meta).
7. **Rollback re-exec** : après restauration, retrait de `DAEDALUS_SISR_MANIFEST`
   et `DAEDALUS_UPDATE_URL` puis `execvp` du binaire restauré (argv d'origine) —
...
### Code
- `daedalus-core/src/sisr/health.rs` (nouveau) : `HealthState { Pending, Healthy,
...
- `daedalus-core/src/sisr/resilience.rs` (nouveau) : `BACKUP_SUFFIX=".bak"`,
  `backup_path_for`, `create_backup` (bytes + perms via `AtomicWriter`),
  `restore_backup` (NotFound si snapshot absent), `discard_backup`
  (idempotent).
- `daedalus-core/src/sisr/engine.rs` : `SisrEngine::target_payload_sha256` =
...
- `daedalus-core/src/sisr/mod.rs` : `pub mod health; pub mod resilience;`
- `stub/src/main.rs` (⚠️ crate sensible — validé) :
  ...
  - Nouvelles fonctions : `health_store_dir` (= `cache_dir()/health`),
    `health_policy` (env `DAEDALUS_HEALTH_TIMEOUT_MS`/`DAEDALUS_HEALTH_MAX_ATTEMPTS`),
...
- `stub/tests/health_rollback.rs` (nouveau) — E2E sur le **vrai binaire stub**
  ...
  3. `quarantined_version_is_refused_on_reinstall` : 1er run crashe ⇒
     quarantaine (avec `DAEDALUS_HEALTH_MAX_ATTEMPTS=1`) ; 2e run avec le même
...
rebuild ; dry-run ne modifie pas le binaire) ⇒ **daedalus-core 196 tests**.
...
### Objectif
Exposer SISR dans le CLI `daedalus` et dans le runtime d'auto-update : flags
`--enable-sisr`/`--key`/`--update-url` à `daedalus build`, sous-commande runtime
cachée `--daedalus-update` interceptée par le launcher **avant** transmission des
args à l'app hôte, affichage clair (progress, stats blocs réutilisés vs
téléchargés), docs `cli/daedalus-build.md` + `guides/user-updates.md`.
...
1. **Transport réseau du stub = HTTPS (validé par l'utilisateur)** : le stub
   ...
   `DAEDALUS_SISR_MANIFEST`). Pour Prompt 7, l'utilisateur a choisi que **le stub
   télécharge lui-même** manifeste + chunks via **`ureq` + rustls**
   (dép. ajoutée à `stub/Cargo.toml`). Le chemin `DAEDALUS_SISR_MANIFEST` local
...
3. **Sémantique de `--key` quand `--enable-sisr` est actif** : `--key` signe le
   **manifeste SISR** (et non le binaire) — conforme au prompt (« Clé privée
   ...
   simultanés non supportés dans un seul `daedalus build` (documenté) ; à réviser
   si un cas d'usage l'exige.
4. **Stockage de l'URL d'update** : champ `update_url` ajouté au JSON meta
   (`MetaOptions.update_url` dans `daedalus-core/src/assembly.rs` +
   `Metadata.update_url` dans `stub/src/main.rs`, serde default). Résolution
   au runtime : arg positionnel de `--daedalus-update` > env `DAEDALUS_UPDATE_URL` >
   `meta.update_url`.
...
6. **Interception dans le stub** : lecture de `std::env::args_os()` au début de
   `run()`. `--daedalus-update [URL]` → mode update **terminal** (applique puis
   exit) ; `--daedalus-version` → affiche la version puis exit. Les deux ne sont
...
9. **Pas de dossier `crates/`** : Prompt mentionne
   `crates/daedalus_cli/src/commands/build.rs` et `crates/daedalus_runtime/src/args.rs`
   ⇒ traduits vers la structure réelle : `daedalus-cli/src/commands/build.rs` et
   runtime dans `stub/src/main.rs` (pas de crate `daedalus_runtime`).

### À faire (todo)
1. `build.rs` : + `--enable-sisr`, `--update-url` ; logique SISR
   (`SisrBuildConfig` 64 KiB + `assemble_daedalus_with_sisr` + sign manifeste
   `--key` + 0600 + JSON/dry-run).
2. `daedalus-core`: `MetaOptions.update_url` + `build_meta_json` ; stub
   `Metadata.update_url`.
3. `stub`: interception `--daedalus-update`/`--daedalus-version` (args_os), résolution
   URL, `HttpChunkFetcher` (ureq), progress + stats (stderr), `--daedalus-version`.
...
6. Docs : `cli/daedalus-build.md`, `guides/user-updates.md`, SUMMARY,
...
- **Code** : `build.rs` (+ `--enable-sisr`, `--update-url`, `build_sisr_config`,
  ...
  `daedalus-core/src/assembly.rs` (`MetaOptions.update_url`), `stub/src/main.rs`
...
- **Docs** : `cli/daedalus-build.md` (nouveau), `guides/user-updates.md` (nouveau),
  SUMMARY à jour, `runtime-launcher.md` (plus « network-free » : déclencheur
  `--daedalus-update` + `HttpChunkFetcher`), `incremental-updates.md` (flags
  `--self-update`/`update` → `--enable-sisr`/`--daedalus-update`).
...
- Modules dans `daedalus-core/src/sisr/` (`engine.rs` + `swap.rs`), pas de crate
  `daedalus_runtime` (convention « pas de `crates/` » ; le stub dépend déjà de
  `daedalus-core`).
- Câblage dans `stub/src/main.rs` via variable d'environnement
  `DAEDALUS_SISR_MANIFEST` (pas de nouvelle commande CLI — Prompt 7).

### Code
- `daedalus-core/src/sisr/mod.rs` (nouveau) : `pub mod engine; pub mod swap;`
- `daedalus-core/src/sisr/engine.rs` (nouveau) : trait `ChunkFetcher`,
...
- `daedalus-core/src/sisr/swap.rs` (nouveau) : `AtomicWriter` RAII (`.tmp-<pid>`,
  flush + `sync_all` + `rename` ; Drop supprime si non commité) +
  `atomic_replace`.
- `daedalus-core/src/lib.rs` : `pub mod sisr;`
- `daedalus-core/src/sisr_stage.rs` : + `RemoteManifest::verify_any(&[VerifyingKey])`
...
- `stub/src/main.rs` :
  - `read_from(path)` (extrait de `read_self`) ; flux : `read_from(/proc/self/exe)`
    → si `DAEDALUS_SISR_MANIFEST` : parse `RemoteManifest` → vérif signature
...
Harness `/tmp/opencode/daedalus-e2e` (hors repo, dépendance path à daedalus-core).
...
### Code
- `daedalus-core/src/sisr_stage.rs` (nouveau) :
...
- `daedalus-core/src/lib.rs` : `pub mod sisr_stage;`
...
- `docs/src/spec/daedalus-format-v2.md` : signature sur `merkle_root ‖ manifest`,
...
### Code
- `daedalus-core/src/sisr_header.rs` : `SisrFooterExt` (110 o : `sisr_version:u16`,
...
- `daedalus-core/src/manifest.rs` : `DeltaManifest` binaire compact (magic `XBMD`,
...
- `daedalus-core/src/format.rs` : `FLAG_SISR = 0x04`, `SISR_FOOTER_EXT_SIZE = 110`,
  `Footer::footer_size()` (84 v2 / 92 v3+), `Footer::has_sisr()`.
- `daedalus-core/src/lib.rs` : `pub mod manifest; pub mod sisr_header;`
...
### Docs
- `docs/src/spec/daedalus-format-v2.md` (créé) : layout physique, table d'octets,
...
- `daedalus-core/src/chunker.rs` : trait `Chunker`, `FastCDC{min,avg,max}`, table
  gear déterministe.
- `daedalus-core/src/cas.rs` : trait `ObjectStore`, `MemoryStore`, `DiskObjectStore`
  (vérification SHA-256 en lecture/écriture).
- `daedalus-core/src/assembler.rs` : trait `BinaryAssembler`, `XbinStitcher`.
...
- Vérif avant clôture : `cargo fmt --check` → `cargo clippy -p <crate>
  ...
  `mdbook build docs/`.
- Zéro `unsafe` dans `daedalus-core`/`daedalus-cli` ; `unsafe` (stub) avec commentaires
  `SAFETY`.
