//! Process execution for the daedalus launcher stub.
//!
...
pub fn setup_env(
    ...
    // Strip all DAEDALUS_* operational env vars inherited from the host so
    // they don't leak into the child application's environment. The stub's
    // own vars (DAEDALUS_TOKEN, DAEDALUS_REGISTRY, DAEDALUS_UPDATE_URL,
    // DAEDALUS_SISR_MANIFEST, etc.) are for the launcher, not the app.
    // Intentional vars (DAEDALUS_ORIG_CWD, DAEDALUS_SECRET_*) are inserted
    // below, after this strip.
    env.retain(|k, _| !k.starts_with("DAEDALUS_"));
    ...
    for (k, v) in daedalus_core::dotenv::load_dotenv(rootfs, None, false) {
    ...
    if let Some(secrets) = &app_config.secrets {
        for (k, v) in secrets {
            env.insert(format!("DAEDALUS_SECRET_{}", k.to_uppercase()), v.clone());
        }
    }
    ...
    for dir in &["/app/data", "/app/tmp"] {
        ...
        if let Err(e) = std::fs::create_dir_all(&path) {
            eprintln!(
                "[daedalus] warning: could not create {}: {e}",
                path.display()
            );
        }
    }
...
fn bind_mount_gui_devices(rootfs: &Path) -> io::Result<()> {
    ...
    for (src, dest_rel) in mounts {
        ...
        if rc != 0 {
            eprintln!(
                "[daedalus] warning: failed to bind-mount {} → {}: {}",
                src,
                dest.display(),
...
fn enter_pivot_sandbox(rootfs: &Path, meta: &Metadata) -> io::Result<()> {
    ...
    let caps = meta.effective_capabilities();

    if meta.seccomp {
        crate::seccomp::install_seccomp_with_capabilities(caps).map_err(|e| {
            io::Error::new(
                io::ErrorKind::Other,
    ...
    if let Some(root) = root_guard {
        crate::landlock::sandbox_with_capabilities(&root, caps).map_err(|e| {
            io::Error::new(
                io::ErrorKind::Other,
    ...
    #[cfg(unix)]
    {
        ...
        if !entrypoint_is_executable(check_path, &interpreter_name, rootfs) {
            return Err(io::Error::new(
                ...
                format!(
                    "[daedalus] error: interpreter '{}' not found (tried: {})",
                    check_name,
                    prog.display()
                ),
            ));
        ...
        if direct_exec {
            ...
            if !entrypoint_is_executable(interpreter_name.as_bytes(), &interpreter_name, rootfs) {
                return Err(io::Error::new(
                    io::ErrorKind::NotFound,
                    format!(
                        "[daedalus] error: interpreter '{}' not found",
                        interpreter_name
                    ),
                ));
            }
        ...
        // Phase 5b: Exec capability enforcement.
        //
        // When `Capability::Exec` is absent, the seccomp filter installed by
        // `enter_pivot_sandbox` denies `execve`. The stub's own `execvp` would
        // be killed by `SECCOMP_RET_KILL_PROCESS`, so we `fork()` first: the
        ...
        let caps = meta.effective_capabilities();
        let has_exec = caps.contains(&Capability::Exec);
        let layers_active = !meta.layers.is_empty() && meta.entrypoint_layer.is_some();

        #[cfg(target_os = "linux")]
        let seccomp_blocks_exec = layers_active && !has_exec && meta.seccomp && use_pivot;
        #[cfg(not(target_os = "linux"))]
        let seccomp_blocks_exec = false;
        ...
        if seccomp_blocks_exec {
            // SAFETY: fork(2) creates a child process. The child attempts
            // libc_execvp which is blocked by seccomp (execve denied).
            // The parent waits for the child via waitpid.
            ...
            return Err(io::Error::new(
                io::ErrorKind::Other,
                format!("unexpected child exit status: {}", status),
            ));
        }

        // Exec capability present, or seccomp not active: standard execvp.
...
pub fn resolve_entrypoint(
    ...
    let runtime_str = meta.effective_runtime().to_string();
    let entrypoint_vec = meta.effective_entrypoint().to_vec();
    if entrypoint_vec.is_empty() {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            "empty entrypoint",
        ));
...
pub fn spawn_app_windows(
    ...
    if !is_executable_path(&cmd) {
        cmd = find_in_bin_paths(rootfs, &entrypoint[0]).ok_or_else(|| {
            io::Error::new(
                io::ErrorKind::NotFound,
                format!(
                    "[daedalus] error: interpreter '{}' not found (tried: {})",
...
pub fn supervise_services(
    ...
    for svc in &meta.services {
        ...
        if !is_executable_path(&prog) {
            prog = find_in_bin_paths(rootfs, &svc.cmd[0]).ok_or_else(|| {
                io::Error::new(
                    io::ErrorKind::NotFound,
                    format!("[daedalus] error: service '{}' not found", svc.cmd[0]),
                )
            })?;
        }
    ...
    for (name, child) in &children {
        let code = crate::win::wait(child)?;
        if verbose {
            eprintln!("[daedalus] service '{}' exited with code {}", name, code);
        }
...
pub fn fork_services(
    ...
    for svc in &meta.services {
        ...
        unsafe {
            ...
            if verbose {
                eprintln!("[daedalus] service '{}' started (pid {})", svc.name, pid);
            }
...
pub fn wait_for_health(meta: &Metadata, verbose: bool) -> io::Result<()> {
    for svc in &meta.services {
        ...
        if verbose {
            eprintln!(
                "[daedalus] waiting for {}:{} (timeout {}s)",
                svc.name, svc.ready_port, timeout
            );
        }
        wait_for_port(svc.ready_port, timeout)?;
        if verbose {
            eprintln!("[daedalus] {}:{} is ready", svc.name, svc.ready_port);
        }
    }
    Ok(())
}
...
pub fn wait_for_children(children: &[(String, i32)], verbose: bool) -> io::Result<()> {
    ...
    while remaining > 0 {
        ...
        if libc::WIFEXITED(status) {
            ...
            eprintln!("[daedalus] service '{}' killed by signal {}", name, sig);
            if exit_code == 0 {
                exit_code = 128 + sig;
            }
...
mod tests {
    ...
    fn expand_env_arg_supports_braces() {
        ...
        assert_eq!(
            expand_env_arg("-Dserver.port=${DAEDALUS_PORT}", &env),
    ...
    /// Chaos: verify that DAEDALUS_* operational env vars from the host do NOT
    ...
    fn daedalus_env_vars_are_stripped_from_child() {
        ...
        std::env::set_var("DAEDALUS_TOKEN", "host_secret_token");
        std::env::set_var("DAEDALUS_REGISTRY", "https://evil.example.com");
        std::env::set_var("DAEDALUS_SISR_ALLOW_UNSIGNED", "1");

        let tmp = tempfile::tempdir().unwrap();
        let rootfs = tmp.path();
        let meta: crate::Metadata = serde_json::from_str(
            r#"{"name":"test","runtime":"python","entrypoint":["python3","app.py"]}"#,
        )
        .unwrap();

        let mut secrets = std::collections::HashMap::new();
        secrets.insert("DB_PASS".to_string(), "config_secret".to_string());
        ...
        assert!(
            !env.contains_key("DAEDALUS_TOKEN"),
            "host DAEDALUS_TOKEN leaked to child"
        );
        assert!(
            !env.contains_key("DAEDALUS_REGISTRY"),
            "host DAEDALUS_REGISTRY leaked to child"
        );
        assert!(
            !env.contains_key("DAEDALUS_SISR_ALLOW_UNSIGNED"),
            "host DAEDALUS_SISR_ALLOW_UNSIGNED leaked to child"
        );
        assert_eq!(
            env.get("DAEDALUS_SECRET_DB_PASS"),
            Some(&"config_secret".to_string()),
            "config secrets should still be injected"
        );

        std::env::remove_var("DAEDALUS_TOKEN");
        std::env::remove_var("DAEDALUS_REGISTRY");
        std::env::remove_var("DAEDALUS_SISR_ALLOW_UNSIGNED");
    }
}
