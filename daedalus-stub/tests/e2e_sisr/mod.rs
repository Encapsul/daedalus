...
//! binary (`CARGO_BIN_EXE_daedalus-stub`, embedded into a freshly built `.daedalus`):
//!
//! ```text
//! 1. build app_v1.daedalus (SISR enabled)
//! 2. start a mock HTTP server serving the v1 → v2 delta (XBMR + chunks)
//! 3. run ./app_v1.daedalus --daedalus-update=http://127.0.0.1:<port>
//! 4. assert app_v1.daedalus became v2 and produces v2's output
//! ```
...
use daedalus_core::sisr_stage::{build_artifacts, RemoteManifest, SisrBuildConfig};
...
pub fn build_daedalus(work: &Path, stub: &Path, body: &str, shared: &[u8]) {
...
fn assemble(work: &Path, stub: &Path, body: &str, shared: &[u8], config: &SisrBuildConfig) {
...
/// A running test environment: an isolated workdir, the built `.daedalus`, a mock
...
pub fn env(v1: &str, v2: &str) -> TestEnv {
    ...
    build_daedalus(&work, &stub, v1, &shared);
    let v1_sha = footer_sha(&work.join("app.daedalus"));
    let staged = stage_update(&work.join("app.daedalus"), v2, &shared, key());
    assert!(
...
/// Runs `./app.daedalus --daedalus-update=<base>` in the isolated environment.
pub fn run_update(env: &TestEnv) -> Output {
    Command::new(&env.app)
        .arg(format!("--daedalus-update={}", env.base_url))
        .env("DAEDALUS_TRUSTED_DIR", env.work.join("trusted"))
        .env("XDG_CACHE_HOME", env.work.join("cache"))
        .env("XDG_DATA_HOME", env.work.join("data"))
        .env("DAEDALUS_HEALTH_TIMEOUT_MS", "3000")
        .output()
        .expect("failed to spawn daedalus --daedalus-update")
}
...
pub fn run_app(env: &TestEnv) -> Output {
    Command::new(&env.app)
        ...
        .env("DAEDALUS_HEALTH_TIMEOUT_MS", "3000")
        .output()
        .expect("failed to spawn the daedalus app")
}
...
/// `[daedalus] update applied: N chunks reused (…), M chunks fetched (…), total K`.
