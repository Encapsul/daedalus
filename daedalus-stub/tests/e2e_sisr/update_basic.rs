...
/// HTTP server, `--daedalus-update`, then assert the binary became v2.
#[cfg(unix)]
#[test]
fn remote_update_swaps_in_v2_and_reports_reuse() {
    ...
    assert!(
        out.status.success(),
        "--daedalus-update must succeed: {stderr}"
    );
...
/// Roadmap #45 anchor at e2e level: the binary rebuilt by a REAL update
/// carries the remote manifest's verified signature, so it stays verifiable
/// at rest instead of degrading to integrity-only.
#[test]
fn updated_binary_carries_the_publisher_signature() {
    let e = env(BODY_V1, BODY_V2);
    let update = run_update(&e);
    assert!(
        update.status.success(),
    ...
    let data = std::fs::read(&e.app).unwrap();
    let (ext, _) = daedalus_core::sisr_header::read_sisr(&mut std::io::Cursor::new(&data))
        .unwrap()
        .expect("updated binary must embed a SISR section");
    assert_ne!(ext.signature, [0u8; 64], "signature must not be zeroed");
    assert_eq!(
        ext.signature, e.staged.remote.signature,
        "rebuilt binary must carry the verified remote signature"
    );
}

/// Non-regression: the local `$DAEDALUS_SISR_MANIFEST` staging path (mission 6)
...
fn local_staging_path_still_applies() {
    ...
    let out = std::process::Command::new(&e.app)
        .env("DAEDALUS_SISR_MANIFEST", &manifest_path)
        .env("DAEDALUS_TRUSTED_DIR", e.work.join("trusted"))
        .env("XDG_CACHE_HOME", e.work.join("cache"))
        .env("XDG_DATA_HOME", e.work.join("data"))
        .env("DAEDALUS_HEALTH_TIMEOUT_MS", "3000")
        .output()
        .expect("failed to spawn daedalus with DAEDALUS_SISR_MANIFEST");
    let stdout = String::from_utf8_lossy(&out.stdout).into_owned();
    let stderr = String::from_utf8_lossy(&out.stderr).into_owned();
