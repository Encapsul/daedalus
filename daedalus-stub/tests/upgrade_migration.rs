...
fn isolated_env(work: &Path) -> Vec<(&str, PathBuf)> {
    vec![
        ("DAEDALUS_TRUSTED_DIR", work.join("trusted")),
...
fn build_legacy(work: &Path, stub: &Path, body: &str, shared: &[u8]) -> PathBuf {
    let out = work.join("legacy.daedalus");
...
fn upgraded_binary_gains_auto_update() {
    ...
    let upgraded = work.join("upgraded.daedalus");
    ...
    let out = Command::new(&upgraded)
        .arg(format!("--daedalus-update={base_url}"))
        .envs(isolated_env(&work))
        .env("DAEDALUS_HEALTH_TIMEOUT_MS", "3000")
        .output()
        .expect("failed to spawn upgraded binary --daedalus-update");
    assert!(
        out.status.success(),
