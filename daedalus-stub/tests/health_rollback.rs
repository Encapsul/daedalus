...
//! `.daedalus`) through the full SISR update flow and exercise the mission 8
//! guarantees:
//!
...
fn build_daedalus(work: &Path, stub: &Path, body: &str) {
...
/// Runs the `.daedalus` with the SISR manifest staged, in an isolated cache.
fn run_daedalus(work: &Path, app: &Path, manifest: &Path) -> Output {
    Command::new(app)
        .env("DAEDALUS_SISR_MANIFEST", manifest)
        .env("DAEDALUS_TRUSTED_DIR", work.join("trusted"))
        .env("XDG_CACHE_HOME", work.join("cache"))
        .env("XDG_DATA_HOME", work.join("data"))
        .env("DAEDALUS_HEALTH_MAX_ATTEMPTS", "1")
        .env("DAEDALUS_HEALTH_TIMEOUT_MS", "3000")
        .output()
        .expect("failed to spawn the daedalus launcher")
}

fn health_store(work: &Path) -> HealthStore {
    HealthStore::new(&work.join("cache").join("daedalus").join("health"))
}
...
fn crashing_update_is_rolled_back_and_old_version_runs() {
    ...
    build_daedalus(&work, &stub, "#!/bin/sh\necho v1-ok; exit 0");
    let v1_sha = footer_sha(&work.join("app.daedalus"));
    let (manifest, v2_sha) = stage_update(
        &work,
        &work.join("app.daedalus"),
        "#!/bin/sh\necho v2-crash; exit 1",
    );

    let out = run_daedalus(&work, &work.join("app.daedalus"), &manifest);
    ...
    assert_eq!(
        footer_sha(&work.join("app.daedalus")),
        v1_sha,
        "on-disk binary must be restored to the previous version"
    );
    assert!(
        !work.join("app.daedalus.bak").exists(),
...
fn quarantined_version_is_refused_on_reinstall() {
    ...
    let first = run_daedalus(&work, &work.join("app.daedalus"), &manifest);
    assert!(
    ...
    let second = run_daedalus(&work, &work.join("app.daedalus"), &manifest);
